pub mod avr_control {
    ...
    pub fn reading_input_registers(client: &mut TcpClient) {
        ...
        println!("Response IR mains_power_supply: {:?}", mains_power_supply_response);
    
        let start_generator_response = client.read_input_registers(00003, 1);
        println!("Response IR start_generator: {:?}", start_generator_response);
    
        let generator_faulty_response = client.read_input_registers(00005, 1);
        println!("Response IR generator_faulty: {:?}", generator_faulty_response);
        ...
        for row in client.query("SELECT нагрузка FROM нагрузка_на_генератор ORDER BY время_и_дата DESC limit 1", &[]).unwrap() {
    ...
    pub fn avr_control_insert() {
        ...
        match result {
            Err(message) => println!("Ошибка! Связь ПЛК с модулем modbus_ats отсутствует! {}", message),
            Ok(_) => {
